		...
		if($config['total_rows'] > 0){
			...
			if(isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)){ 
				foreach($listCatalogue as $key => $val){ 
					...
					$html = $html .'<a type="button" class="btn btn-danger btn-delete ajax-delete" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ bài viết trong nhóm này sẽ bị xóa. Hãy chắc chắn rằng bạn muốn thực hiện hành động này!" data-id="'.$val['id'].'" data-module="article_catalogue"><i class="fa fa-trash"></i></a> ';
					$html = $html .'</td> ';
					$html = $html .'</tr>';
				}
			}
