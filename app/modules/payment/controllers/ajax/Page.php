...
class Page extends MY_Controller {
	...
	public function listpage(){
		...
		$extend = '';
		if(!in_array('page/backend/page/viewall', json_decode($this->auth['permission'], TRUE))){
		...
		if($config['total_rows'] > 0){
			...
			if($catalogueid > 0){
				$listCatalogue = $this->Autoload_Model->_condition(array(
					'module' => 'page',
					'select' => '`object`.`id`, `object`.`title`,  `object`.`image`, `object`.`catalogueid`, `object`.`catalogue`, `object`.`publish`, `object`.`created`, `object`.`order`, `object`.`viewed`, (SELECT fullname FROM user WHERE user.id = object.userid_created) as user_created, (SELECT title FROM page_catalogue WHERE page_catalogue.id = object.catalogueid) as catalogue_title',
					'where' => ((!empty($extend)) ? '`object`.`userid_created` = '.$this->auth['id'].'' : ''),
					'keyword' => '(`object`.`title` LIKE \'%'.$keyword.'%\' AND `object`.`description` LIKE \'%'.$keyword.'%\')',
					'catalogueid' => $catalogueid,
					'limit' => $perpage,
			...
			foreach($listCatalogue as $key => $val){
				...
				$html = $html .'<tr class="gradeX">';
					...
					$html = $html.'<td>';
						$html = $html.'<div class="uk-flex uk-flex-middle">';
							...
							$html = $html.'<div class="main-info">';
								$html = $html.'<div class="title"><a class="maintitle" href="'.site_url('page/backend/page/update/'.$val['id'].'').'" title="">'.$val['title'].' ('.$val['viewed'].')</a></div>';
								$html = $html.'<div class="catalogue" style="font-size:10px">';
									$html = $html.'<span style="color:#f00000;">Nhóm hiển thị: </span>';
									$html = $html.'<a class="" style="color:#333;" href="'.site_url('page/backend/page/view?catalogueid='.$val['catalogueid']).'" title="">'.$val['catalogue_title'].'</a>'.((isset($_catalogue_list) && is_array($_catalogue_list) && count($_catalogue_list) ) ? ' ,' :'').'';
									if(isset($_catalogue_list) && is_array($_catalogue_list) && count($_catalogue_list)){ foreach($_catalogue_list as $keyCat => $valCat){ 
										$html = $html.'<a style="color:#333;" class="" href="'.site_url('page/backend/page/view?catalogueid='.$valCat['id']).'" title="">'.$valCat['title'].'</a> '.(($keyCat + 1 < count($_catalogue_list)) ? ', ' : '').'';
									}} 
								$html = $html.'</div>';
							$html = $html.'</div>';
						$html = $html.'</div>';
					...
					$html = $html.'<td class="text-center">';
						$html = $html.'<a type="button" href="'.(site_url('page/backend/catalogue/update/'.$val['id'].'')).'" class="btn btn-sm btn-primary mr5"><i class="fa fa-edit"></i></a>';
						$html = $html.'<a type="button" class="btn btn-sm btn-danger ajax-delete" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ nội dung tĩnh trong nhóm này sẽ bị xóa. Hãy chắc chắn bạn muốn thực hiện chức năng này!" data-id="'.$val['id'].'" data-module="page"><i class="fa fa-trash"></i></a>';
					$html = $html.'</td>';
				$html = $html.'</tr>';
			 }
		}else{ 
