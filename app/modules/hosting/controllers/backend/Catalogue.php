...
class Catalogue extends MY_Controller {
	...
	public function view($page = 1){
		$this->commonbie->permission("hosting/backend/catalogue/view", $this->auth['permission']);
		$page = (int)$page;
		$data['from'] = 0;
		$data['to'] = 0;
		...
		if($config['total_rows'] > 0){
			...
			$config['base_url'] = base_url('hosting/backend/catalogue/view');
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, publish, created, order, level, lft, rgt, (SELECT fullname FROM user WHERE user.id = hosting_catalogue.userid_created) as user_created',
				'table' => 'hosting_catalogue',
		...
		$data['script'] = 'hosting_catalogue';
		$data['config'] = $config;
		$data['template'] = 'hosting/backend/catalogue/view';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Create(){
		$this->commonbie->permission("hosting/backend/catalogue/create", $this->auth['permission']);
				...
				if($resultid > 0){
					...
					if(!empty($canonical)){
						$router = array(
							...
							'uri' => 'hosting/frontend/catalogue/view',
							'param' => $resultid,
							'type' => 'number',
							'created' => gmdate('Y-m-d H:i:s', time() + 7*3600),
						);
					...
					redirect('hosting/backend/catalogue/create');
				}
			}
		}
		
		$data['template'] = 'hosting/backend/catalogue/create';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Update($id = 0){
		$this->commonbie->permission("hosting/backend/catalogue/update", $this->auth['permission']);
		$id = (int)$id;
		$detailCatalogue = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, description, meta_title, meta_description, parentid, image, publish',
			'table' => 'hosting_catalogue',
			'where' => array('id' => $id),
		));
		if(!isset($detailCatalogue) || is_array($detailCatalogue) == false || count($detailCatalogue) == 0){
			...
			redirect('hosting/backend/catalogue/view');
		}
				...
				if($flag > 0){
					...
					if(!empty($canonical)){
						$this->Autoload_Model->_delete(array(
							'where' => array('canonical' => $detailCatalogue['canonical'],'uri' => 'hosting/frontend/catalogue/view','param' => $id),
							'table' => 'router',
						));
						$router = array(
							'canonical' => $canonical,
					...
					redirect('hosting/backend/catalogue/update/'.$id.'');
				}
			}
		}
		...
		$data['template'] = 'hosting/backend/catalogue/update';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
