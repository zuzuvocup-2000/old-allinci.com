<?php if(isset($mediaList) && is_array($mediaList) && count($mediaList)){ ?>
	...
	<?php if(isset($style) && $style == 'style-1'){ ?>
		<ul class="uk-grid uk-grid-medium uk-grid-width-1-2 uk-grid-width-medium-1-3 uk-grid-width-large-1-3 _list_media" data-uk-grid-match="{target: '.ht2109_grid_match'}">
			<?php foreach ($mediaList as $keyPrj => $valPrj) { ?>
			 	<?php        
			        $title = $valPrj['title'];
                    ...
                    <div class="uk-clearfix _media">
                        ...
                        <div class="info">
                            ...
                            */ ?>
			    			...
			    			<div class="ht2109_readmore"><a href="<?php echo $href ?>">Read More</a></div>
		...
		<?php 
	 	...
	 	<section class="owl-slide mb-common">
			<div class="uk-container uk-container-center">
				<div class="owl-carousel owl-theme _list_media style-2" data-option="<?php echo base64_encode(json_encode($owlInit)); ?>">
					<?php foreach ($mediaList as $keyPost => $valPost) {?>
						<?php 								
							$title = $valPost['title'];
	...
	<?php }else{ ?>
		<ul class="uk-grid uk-grid-medium uk-grid-width-1-2 uk-grid-width-medium-1-3 uk-grid-width-large-1-4 _list_media" data-uk-grid-match="{target: '.ht2109_grid_match'}">
			<?php foreach ($mediaList as $keyPost => $valPost) {?>
				<?php 								
					$title = $valPost['title'];
						...
						<div class="thumb">
							...
							*/ ?>
							<div class="overlay-thumb">
								<a href="<?php echo $href ?>" title="<?php echo $title ?>" class="title"><?php echo $title ?></a>
							</div>
						</div>
						<?php /*
						<div class="info uk-clearfix ht2109_grid_match">
