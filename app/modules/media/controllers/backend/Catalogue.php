...
class Catalogue extends MY_Controller {
	...
	public function view($page = 1){
		$this->commonbie->permission("media/backend/catalogue/view", $this->auth['permission']);
		$page = (int)$page;
		$data['from'] = 0;
		$data['to'] = 0;
		...
		if($config['total_rows'] > 0){
			...
			$config['base_url'] = base_url('media/backend/catalogue/view');
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, publish, created, order, level, lft, rgt, (SELECT fullname FROM user WHERE user.id = media_catalogue.userid_created) as user_created, ishome, highlight',
				'table' => 'media_catalogue',
		...
		$data['script'] = 'media_catalogue';
		$data['config'] = $config;
		$data['template'] = 'media/backend/catalogue/view';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Create(){
		$this->commonbie->permission("media/backend/catalogue/create", $this->auth['permission']);
		if($this->input->post('create')){
			$this->load->library('form_validation');
			$this->form_validation->CI =& $this;
			$this->form_validation->set_error_delimiters('','/');
			$this->form_validation->set_rules('title', 'Tiêu đề danh mục', 'trim|required');
			$this->form_validation->set_rules('layoutid', 'Loại Thư viện', 'trim|is_natural_no_zero');
				...
				if($resultid > 0){
					...
					if(!empty($canonical)){
						$router = array(
							...
							'uri' => 'media/frontend/catalogue/view',
							'param' => $resultid,
							'type' => 'number',
							'created' => gmdate('Y-m-d H:i:s', time() + 7*3600),
						);
					...
					redirect('media/backend/catalogue/create');
				}
			}
		}
		
		$data['template'] = 'media/backend/catalogue/create';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Update($id = 0){
		$this->commonbie->permission("media/backend/catalogue/update", $this->auth['permission']);
		$id = (int)$id;
		$detailCatalogue = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, description, meta_title, meta_description, parentid, layoutid, image, publish',
			'table' => 'media_catalogue',
			'where' => array('id' => $id),
		));
		if(!isset($detailCatalogue) || is_array($detailCatalogue) == false || count($detailCatalogue) == 0){
			...
			redirect('media/backend/catalogue/view');
		}
				...
				if($flag > 0){
					...
					if(!empty($canonical)){
						$this->Autoload_Model->_delete(array(
							'where' => array('canonical' => $detailCatalogue['canonical'],'uri' => 'media/frontend/catalogue/view','param' => $id),
							'table' => 'router',
						));
						$router = array(
							'canonical' => $canonical,
					...
					redirect('media/backend/catalogue/update/'.$id.'');
				}
			}
		}
		...
		$data['template'] = 'media/backend/catalogue/update';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
