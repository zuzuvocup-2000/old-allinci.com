						...
						<div class="table-responsive">
							<div class="uk-flex uk-flex-middle uk-flex-space-between">
								<div class="perpage">
									<div class="uk-flex uk-flex-middle mb10">
										<?php echo form_dropdown('perpage', $this->configbie->data('perpage'), set_value('perpage',$this->input->get('perpage')) ,'class="form-control input-sm perpage filter"  data-url="'.site_url('article/backend/article/view').'"'); ?>
									</div>
								</div>
								<div class="toolbox">
									<div class="uk-flex uk-flex-middle uk-flex-space-between">
										...
										<div class="uk-button">
											<a href="<?php echo site_url('article/backend/article/create'); ?>" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i> Thêm bài viết mới</a>
										</div>
									</div>
								</div>
							</div>
							...
							<table class="table table-striped table-bordered table-hover dataTables-example" >
								...
								<tbody id="ajax-content">
									<?php if(isset($listArticle) && is_array($listArticle) && count($listArticle)){ ?>
										<?php foreach($listArticle as $key => $val){ ?>
										<?php 
											...
											<tr class="gradeX" id="post-<?php echo $val['id']; ?>">
												...
												<td>
													<div class="uk-flex uk-flex-middle">
														...
														<div class="main-info">
															<div class="title"><a class="maintitle" href="<?php echo site_url('article/backend/article/update/'.$val['id']); ?>" title=""><?php echo $val['title']; ?> (<?php echo $val['viewed']; ?>)</a></div>
												...
												<td>
													<?php echo form_input('order['.$val['id'].']', $val['order'], 'data-module="article" data-id="'.$val['id'].'"  class="form-control sort-order" placeholder="Vị trí" style="width:50px;text-align:right;"');?>
												</td>
												...
												<td class="text-center">
													<a type="button" href="<?php echo site_url('article/backend/article/update/'.$val['id'].'') ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
													<a type="button" class="btn btn-danger ajax-delete" data-title="Lưu ý: Dữ liệu sẽ không thể khôi phục. Hãy chắc chắn rằng bạn muốn thực hiện hành động này!" data-router="<?php echo $val['canonical']; ?>" data-id="<?php echo $val['id'] ?>" data-module="article"><i class="fa fa-trash"></i></a>
												</td>
											</tr>
										<?php }}else{ ?>
											<tr>
