							...
							<div class="uk-flex uk-flex-middle uk-flex-space-between">
								...
								<div class="toolbox">
									<div class="uk-flex uk-flex-middle uk-flex-space-between">
										...
										<div class="uk-button">
											<a href="<?php echo site_url('article/backend/catalogue/create'); ?>" class="btn btn-danger"><i class="fa fa-plus"></i> Thêm danh mục bài viết mới</a>
										</div>
									</div>
								</div>
							</div>
									...
									<?php if(isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)){ ?>
										...
										?>
											<tr class="gradeX" id="cat-<?php echo $val['id']; ?>">
												...
												<td><a class="maintitle" style="<?php echo ($val['level'] == 1) ? 'font-weight:600;' : ''; ?>" href="<?php echo site_url('article/backend/article/view?catalogueid='.$val['id'].''); ?>" title=""><?php echo str_repeat('|----', (($val['level'] > 0)?($val['level'] - 1):0)).$val['title']; ?> (<?php echo $count; ?>)</a></td>
												<td>
													<?php echo form_input('order['.$val['id'].']', $val['order'], 'data-module="article_catalogue" data-id="'.$val['id'].'"  class="form-control sort-order" placeholder="Vị trí" style="width:50px;text-align:right;"');?>
												</td>
												...
												<td class="text-center">
													<a type="button" href="<?php echo site_url('article/backend/catalogue/update/'.$val['id'].'') ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
													<a <?php echo ($val['rgt'] - $val['lft'] > 1) ? 'disabled="disabled" ': ''; ?> type="button" class="btn btn-danger <?php echo ($val['rgt'] - $val['lft'] > 1) ? '" ': 'ajax-delete'; ?>" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ bài viết trong nhóm này sẽ bị xóa. Hãy chắc chắn rằng bạn muốn thực hiện hành động này!" data-router="<?php echo $val['canonical']; ?>" data-id="<?php echo $val['id'] ?>" data-module="article_catalogue" data-child="1"><i class="fa fa-trash"></i></a>
												</td>
											</tr>
										<?php }}else{ ?>
											<tr>
