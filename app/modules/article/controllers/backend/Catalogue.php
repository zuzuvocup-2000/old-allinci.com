...
class Catalogue extends MY_Controller {
	...
	public function view($page = 1){
		$this->commonbie->permission("article/backend/catalogue/view", $this->auth['permission']);
		$page = (int)$page;
		$data['from'] = 0;
		$data['to'] = 0;
		...
		if($config['total_rows'] > 0){
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, ishome, highlight, publish, created, order, level, lft, rgt, (SELECT fullname FROM user WHERE user.id = article_catalogue.userid_created) as user_created',
				'table' => 'article_catalogue',
		...
		$data['script'] = 'article_catalogue';
		$data['config'] = $config;
		$data['template'] = 'article/backend/catalogue/view';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Create(){
		$this->commonbie->permission("article/backend/catalogue/create", $this->auth['permission']);
		if($this->input->post('create')){
			...
			if($this->form_validation->run($this)){
				...
				if($resultid > 0){
					...
					redirect('article/backend/catalogue/create');
				}
			}
		}
		$data['script'] = 'article_catalogue';
		$data['template'] = 'article/backend/catalogue/create';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Update($id = 0){
		$this->commonbie->permission("article/backend/catalogue/update", $this->auth['permission']);

		$id = (int)$id;
		$detailCatalogue = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, excerpt, description, icon, image, album, meta_title, meta_description, parentid, image, publish, url_view, landing_link',
			'table' => 'article_catalogue',
			'where' => array('id' => $id),
		));
		if(!isset($detailCatalogue) || is_array($detailCatalogue) == false || count($detailCatalogue) == 0){
			...
			redirect('article/backend/catalogue/view');
		}
				...
				if($flag > 0){
					...
					if(!empty($canonical)){
						$this->Autoload_Model->_delete(array(
							'where' => array('canonical' => $detailCatalogue['canonical'],'uri' => 'article/frontend/catalogue/view','param' => $id),
							'table' => 'router',
						));
						$router = array(
							'canonical' => $canonical,
					...
					// redirect('article/backend/catalogue/update/'.$id.'');
					redirect('article/backend/catalogue/view');
		...
		$data['script'] = 'article_catalogue';
		$data['template'] = 'article/backend/catalogue/update';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
