				...
				<div class="col-lg-8 clearfix">
					<div class="ibox mb20">
						...
						<div class="ibox-content">
							...
							<div class="row">
								<div class="col-lg-6 m-b-xs">
									<div class="form-row">
										...
										<div class="form-row">
											<?php echo form_dropdown('catalogue', $this->configbie->data('catalogue') , set_value('catalogue'), 'class="form-control select3NotSearch"');?>
										</div>
									</div>
								</div>
								<div class="col-lg-6 m-b-xs">
									<div class="form-row">
										...
										<div class="form-row">
											<?php echo form_dropdown('module', $this->configbie->data('module'), set_value('module'), 'class="form-control m-b select3NotSearch"');?>
										</div>
									</div>
								</div>
								<div class="col-lg-12">
									<div class="uk-flex uk-flex-middle m-b-sm use_common" style="cursor: pointer;">
										<?php $use_common = $this->input->post('use_common'); ?>
							...
							<?php $catalogue = $this->input->post('catalogue'); ?>
							<div class="row mb15 coupon" <?php  echo ($catalogue == 'KM') ? 'style="display:none"' : 'style="display:block"' ; ?> >
								<div class="uk-flex uk-flex-middle m-b-sm">
									<div class="col-lg-6">
										<?php echo form_input('code', set_value('code'), 'placeholder="Nhập Mã coupon" autocomplete="off" class="form-control"');?>
									</div>
									<div class="col-lg-3">
								...
								<div class="col-sm-6">
									...
									<div class="input-group limmit_code">
										<?php $not_limmit = $this->input->post('not_limmit'); ?>
										<?php echo form_input('limmit_code', set_value('limmit_code'), 'placeholder="Nhập số lần sử dụng" autocomplete="off" class="form-control int" '.(($not_limmit == -1) ? 'readonly' : '' ));?>
							...
							<div class="row mb15">
								...
								<div class="col-lg-6 m-b-sm ">
									<div class="form-row">
										<?php echo form_dropdown('discount_type', $this->configbie->data('discount_type') , set_value('discount_type'), 'class="form-control select3NotSearch"');?>
									</div>	
								</div>
								<?php $discount_type = $this->input->post('discount_type'); ?>

								<div class="col-lg-6 m-b discount_block"  >
									<div class="uk-flex uk-flex-middle uk-flex-space-between">
										<div class="m-r extend2"> Giảm</div>
										<div class="uk-flex uk-flex-middle">
											
											<?php $freeship = $this->input->post('freeship'); ?>
											<?php $discount_value = $this->input->post('discount_value'); ?>
											...
											<?php echo form_input('discount_value', set_value('discount_value',0), 'class="form-control int" autocomplete="off" placeholder="'.(($discount_type == 'percent' ) ? 'Nhập phần trăm' : 'Nhập số tiền' )  .'" style="width:100%"'.(($freeship == 1) ? 'readonly' : '' ) );?>
											<div style="font-size:13px; width: 60px; border: 1px solid #c4cdd5;border-left:none;border-radius: 0px" class="btn btn-white extend">
	                                            	<?php echo (($discount_type == 'percent' ) ? '%' : 'VNĐ' ) ?>
	                                        </div>
								...
								<div class="freeship" style="display:<?php echo ($discount_type == 'ship' ) ? 'block' : 'none' ;  ?>">
									<div class="col-lg-12 m-b">
										<div class="i-checks"><label> <input type="radio" value="city" name="ship_choose_local" <?php echo !empty($this->input->post('city[]')) ? 'checked=""' : '' ;  ?>  > <i></i>Chọn tỉnh/ thành phố</label></div>
                                        <div class="i-checks"><label> <input type="radio" value="district" name="ship_choose_local" <?php echo !empty($this->input->post('district[]')) ? 'checked=""' : '' ;  ?> > <i></i> Chọn quận huyện </label></div>
									</div>
									<div class="ship_city" style="display:<?php echo !empty($this->input->post('city[]')) ? 'block' : 'none' ;  ?>">
										<div class="col-lg-9">
											<?php $freeshipAll = $this->input->post('freeshipAll'); ?>
											<div class="form-row">
												<?php echo form_dropdown('city[]', '', '', 'class="form-control  selectMultipe" data-module="vn_province" multiple="multiple" data-title="Nhập 2 kí tự để chọn tỉnh, TP.." data-select="name" data-key="provinceid" data-json="'.base64_encode(json_encode($this->input->post('city[]'))).'" style="width: 100%;"  '.(($freeshipAll == 1) ? 'disabled' : '')); ?>
											</div>
										</div>
									...
									<div class="ship_district col-lg-12"  style="display:<?php echo !empty($this->input->post('district[]')) ? 'block' : 'none' ;  ?>">
										<?php $freeshipAll = $this->input->post('freeshipAll'); ?>
										<div class="form-row">
											<?php echo form_dropdown('district[]', '', '', 'class="form-control  selectMultipe" data-module="vn_district" multiple="multiple" data-title="Nhập 2 kí tự để chọn quận huyên.." data-select="name" data-key="districtid" data-json='.base64_encode(json_encode($this->input->post('district[]'))).'   style="width: 100%;"  '); ?>
										</div>
									</div>
								</div>
								...
								<div class="col-lg-6 m-b-sm discount_moduleid" style="display: <?php echo ($discount_type == 'object' ) ? 'block' : 'none' ;  ?>">
									<div class="form-row">
										<?php echo form_dropdown('discount_moduleid[]', '', '', 'class="form-control selectMultipe" multiple="multiple" data-title ="nhập 2 kí tự để tìm kiếm" data-module="product" '); ?>
									</div>	
								</div>
										...
										<div class="form-row">
												<?php foreach ($discount_moduleid as $key => $val) { ?>
									    			...
									    			<div class="uk-flex uk-flex-middle uk-flex-space-between">
									    				...
									    				<div class="del m-r-sm" data-id="<?php echo $product['id'] ?>"><i class="fa fa-times" aria-hidden="true"></i></div>
									    			</div>
									    		</div>
												<?php } ?>
										</div>
							...
							<div class="row mb15">
								...
								<?php $condition_type = $this->input->post('condition_type'); ?>
								<div class="col-lg-6">
									<div class="form-row">
										<?php echo form_dropdown('condition_type', $this->configbie->data('condition_type') , set_value('condition_type'), 'class="form-control select3NotSearch"');?>
									</div>	
								</div>
								<?php 
									$module = $this->input->post('module');
									$module = (isset($module)) ? $module : "product";
									$module  = ($condition_type == 'condition_moduleid')? $module : $module.'_catalogue';  
								?>
								<div class="col-lg-6 condition_value" style="display: <?php echo ($condition_type == 'condition_moduleid' || $condition_type == 'condition_module_catalogue' ) ? 'block' : 'none' ;  ?>">
									<div class="form-row">
	                                    <?php echo form_dropdown('condition_value[]', '', '', 'class="form-control  selectMultipe" multiple="multiple" data-module="'.$module.'" multiple="multiple" data-title="Nhập 2 kí tự để tìm kiếm.." style="width: 100%;"  id="product_catalogue"'); ?>
									</div>	
								</div>
							</div>
							...
							<div class="row mb15 block_condition_1" <?php  echo ($catalogue == 'KM') ? 'style="display:block"' : 'style="display:none"' ; ?>>
								<div class="col-lg-6  "  >
									<div class="form-row">
										<?php echo form_dropdown('condition_type_1',$this->configbie->data('condition_type_1'), set_value('condition_type_1'), 'class="form-control select3NotSearch"');?>
									</div>	
								</div>
								<div class="col-lg-6 " >
									<div class="form-row">
										<?php echo form_input('condition_value_1', set_value('condition_value_1',0), 'class="form-control int" style="width:100%"');?>
									</div>	
								</div>
							</div>
				...
				<div class="col-lg-4">
					<div class="ibox mb20">
						...
						<div class="ibox-content">
							...
							<?php echo form_input('end_date', set_value('end_date'), 'autocomplete="off" placeholder="Đến ngày" class="form-control datetimepicker m-b"'.(($choose_date == '1' ) ? 'readonly' : ''));?>

							<div class="uk-flex uk-flex-middle m-b choose_date" style="cursor: pointer;">
								<input type="checkbox" name="choose_date" value="1" <?php  echo ($choose_date == 1) ? 'checked' : '' ; ?> class="checkbox-item">
								<div for="" class="label-checkboxitem m-r-xs <?php  echo ($choose_date == 1) ? 'checked' : '' ; ?>"></div>
