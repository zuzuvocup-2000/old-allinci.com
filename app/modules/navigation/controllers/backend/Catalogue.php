...
class Catalogue extends MY_Controller {
	...
	public function Create(){
		$this->commonbie->permission("navigation/backend/catalogue/create", $this->auth['permission']);
		if($this->input->post('create')){
			...
			if($this->form_validation->run($this)){
				...
				if($resultid > 0){
					$this->session->set_flashdata('message-success', 'Tạo vị trí hiển thị Menu mới thành công');
					redirect('navigation/backend/catalogue/create');
				}
			}
		}
		
		$data['template'] = 'navigation/backend/catalogue/create';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Update($id = 0){
		$this->commonbie->permission("navigation/backend/catalogue/update", $this->auth['permission']);
		$id = (int)$id;
		...
		if(!isset($detailCatalogue) || is_array($detailCatalogue) == false || count($detailCatalogue) == 0){
			...
			redirect('navigation/backend/catalogue/view');
		}
		if($this->input->post('update')){
			...
			if($this->form_validation->run($this)){
				...
				if($flag > 0){
					...
					redirect('navigation/backend/catalogue/update/'.$id.'');
				}
			}
		}
		$data['detailCatalogue'] = $detailCatalogue;
		$data['template'] = 'navigation/backend/catalogue/update';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
