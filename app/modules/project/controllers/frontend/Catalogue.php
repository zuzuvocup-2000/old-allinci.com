...
class Catalogue extends MY_Controller {
	...
	public function View($id = 0, $page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$page = $page - 1;
			$data['projectList'] = $this->Autoload_Model->_condition(array(
				'module' => 'project',
				'select' => '`object`.`id`, `object`.`title`,`object`.`canonical`, `object`.`image`, `object`.`created`, `object`.`viewed`, `object`.`description`, (SELECT fullname FROM user WHERE user.id = object.userid_created) as user_created, (SELECT title FROM project_catalogue WHERE project_catalogue.id = object.catalogueid) as catalogue_title, (SELECT count(id) FROM comment WHERE comment.detailid = object.id AND comment.module = \'project\') as comment',
				'where' => '`object`.`publish_time` <= "'.$this->currentTime.'" AND `object`.`publish` = 0',
				'catalogueid' => $id,
		...
		if(isset($detailCatalogue['url_view']) && $detailCatalogue['url_view'] != ''){
			...
			$data['template'] = 'project/frontend/catalogue/view';
		}
