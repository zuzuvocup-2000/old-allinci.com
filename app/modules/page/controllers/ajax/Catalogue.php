	...
	public function listCatalogue(){
		...
		if($config['total_rows'] > 0){
			...
			$listCatalogue = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, publish, created, order, level, (SELECT fullname FROM user WHERE user.id = page_catalogue.userid_created) as user_created',
				'table' => 'page_catalogue',
				'keyword' => '(title LIKE \'%'.$keyword.'%\' OR description LIKE \'%'.$keyword.'%\')',
		 ...
		 if(isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)){ 
			 foreach($listCatalogue as $key => $val){ 
				$html = $html .'<tr class="gradeX">';
					...
					$html = $html.'<td><a class="maintitle" style="'.(($val['level'] == 1) ? 'font-weight:600;' : '').'" href="'.site_url('pages/backend/pages/view?cataloguesid='.$val['id'].'').'" title="">'.(str_repeat('|----------', (($val['level'] > 0)?($val['level'] - 1):0)).$val['title']).' (0)</a></td>';
					$html = $html.'<td>';
					...
					$html = $html.'<td class="text-center">';
						...
						$html = $html.'<a type="button" class="btn btn-sm btn-danger ajax-delete" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ bài viết trong nhóm này sẽ bị xóa. Hãy chắc chắn bạn muốn thực hiện chức năng này!" data-id="'.$val['id'].'" data-module="page_catalogue"><i class="fa fa-trash"></i></a>';
					$html = $html.'</td>';
				$html = $html.'</tr>';
			 }
		}else{ 
