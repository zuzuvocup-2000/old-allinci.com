<div id="page-wrapper" class="gray-bg dashbard-1">
	...
	<div class="row wrapper border-bottom white-bg page-heading">
		<div class="col-lg-10">
			<h2>Quản lý danh mục nội dung tĩnh</h2>
			<ol class="breadcrumb">
				<li>
					<a href="<?php echo site_url('admin'); ?>">Home</a>
				</li>
				<li class="active"><strong>Quản lý danh mục nội dung tĩnh</strong></li>
			</ol>
		</div>
	</div>
			...
			<div class="col-lg-12">
				<div class="ibox float-e-margins">
					<div class="ibox-title">
						<h5>Danh sách danh mục nội dung tĩnh</h5>
						<div class="ibox-tools">
							...
							<ul class="dropdown-menu dropdown-page">
								<li><a type="button" class="ajax-recycle-all" data-title="Lưu ý: Khi bạn xóa danh mục nội dung tĩnh, toàn bộ nội dung tĩnh trong nhóm này sẽ bị xóa. Hãy chắc chắn rằng bạn muốn thực hiện chức năng này!" data-module="page_catalogue">Xóa tất cả</a>
								</li>
							</ul>
							<a class="close-link">
							...
							<div class="uk-flex uk-flex-middle uk-flex-space-between">
								...
								<div class="toolbox">
									<div class="uk-flex uk-flex-middle uk-flex-space-between">
										...
										<div class="uk-button">
											<a href="<?php echo site_url('page/backend/catalogue/create'); ?>" class="btn btn-danger"><i class="fa fa-plus"></i> Thêm danh mục nội dung tĩnh mới</a>
										</div>
									</div>
								</div>
							</div>
									...
									<?php if(isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)){ ?>
										...
										?>
											<tr class="gradeX" id="cat-<?php echo $val['id']; ?>">
												...
												<td><a class="maintitle" style="<?php echo ($val['level'] == 1) ? 'font-weight:600;' : ''; ?>" href="<?php echo site_url('page/backend/page/view?catalogueid='.$val['id'].''); ?>" title=""><?php echo str_repeat('|----', (($val['level'] > 0)?($val['level'] - 1):0)).$val['title']; ?> (<?php echo $count; ?>)</a></td>
												<td>
													<?php echo form_input('order['.$val['id'].']', $val['order'], 'data-module="page_catalogue" data-id="'.$val['id'].'"  class="form-control sort-order" placeholder="Vị trí" style="width:50px;text-align:right;"');?>
												</td>
												...
												<td class="text-center">
													<a type="button" href="<?php echo site_url('page/backend/catalogue/update/'.$val['id'].'') ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
													<a <?php echo ($val['rgt'] - $val['lft'] > 1) ? 'disabled="disabled" ': ''; ?> type="button" class="btn btn-danger <?php echo ($val['rgt'] - $val['lft'] > 1) ? '" ': 'ajax-delete'; ?>" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ nội dung tĩnh trong nhóm này sẽ bị xóa. Hãy chắc chắn rằng bạn muốn thực hiện hành động này!" data-module="page_catalogue" data-child="1" data-id="<?php echo $val['id'] ?>"><i class="fa fa-trash"></i></a>
												</td>
											</tr>
										<?php }}else{ ?>
											<tr>
