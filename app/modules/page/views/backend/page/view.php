<div id="page-wrapper" class="gray-bg dashbard-1">
	...
	<div class="row wrapper border-bottom white-bg page-heading">
		<div class="col-lg-10">
			<h2>Quản lý nội dung tĩnh</h2>
			<ol class="breadcrumb">
				<li>
					<a href="<?php echo site_url('admin'); ?>">Home</a>
				</li>
				<li class="active"><strong>Quản lý nội dung tĩnh</strong></li>
			</ol>
		</div>
	</div>
					...
					<div class="ibox-title">
						...
						<div class="ibox-tools">
							...
							<ul class="dropdown-menu dropdown-page">
								<li><a type="button" class="ajax-delete-all" data-title="Lưu ý: Khi bạn thực hiện thao tác này, toàn bộ dữ liệu sẽ không thể khôi phục được. Hãy chắc chắn rằng bạn muốn thực hiện chức năng này!" data-module="page">Xóa tất cả</a>
								</li>
							</ul>
							<a class="close-link">
						...
						<div class="table-responsive">
							<div class="uk-flex uk-flex-middle uk-flex-space-between">
								<div class="perpage">
									<div class="uk-flex uk-flex-middle mb10">
										<?php echo form_dropdown('perpage', $this->configbie->data('perpage'), set_value('perpage',$this->input->get('perpage')) ,'class="form-control input-sm perpage filter"  data-url="'.site_url('page/backend/page/view').'"'); ?>
									</div>
								</div>
								<div class="toolbox">
									<div class="uk-flex uk-flex-middle uk-flex-space-between">
										...
										<div class="uk-button">
											<a href="<?php echo site_url('page/backend/page/create'); ?>" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i> Thêm nội dung tĩnh mới</a>
										</div>
									</div>
								</div>
							</div>
							...
							<table class="table table-striped table-bordered table-hover dataTables-example" >
								...
								<tbody id="ajax-content">
									<?php if(isset($listpage) && is_array($listpage) && count($listpage)){ ?>
										<?php foreach($listpage as $key => $val){ ?>
										<?php 
											...
											<tr class="gradeX" id="post-<?php echo $val['id']; ?>">
												...
												<td>
													<div class="uk-flex uk-flex-middle">
														...
														<div class="main-info">
															<div class="title"><a class="maintitle" href="<?php echo site_url('page/backend/page/update/'.$val['id']); ?>" title=""><?php echo $val['title']; ?> (<?php echo $val['viewed']; ?>)</a></div>
															<div class="catalogue" style="font-size:10px">
																<span style="color:#f00000;">Nhóm hiển thị: </span>
																<a class="" style="color:#333;" href="<?php echo site_url('page/backend/page/view?catalogueid='.$val['catalogueid']); ?>" title=""><?php echo $val['catalogue_title']; ?></a><?php echo (isset($_catalogue_list) && is_array($_catalogue_list) && count($_catalogue_list) ) ? ' ,' :''; ?>
																<?php if(isset($_catalogue_list) && is_array($_catalogue_list) && count($_catalogue_list)){ foreach($_catalogue_list as $keyCat => $valCat){ ?>
																<a style="color:#333;" class="" href="<?php echo site_url('page/backend/page/view?catalogueid='.$valCat['id']); ?>" title=""><?php echo $valCat['title']; ?></a> <?php echo ($keyCat + 1 < count($_catalogue_list)) ? ', ' : ''; ?>
																<?php }} ?>
															</div>
														</div>
													</div>
												...
												<td>
													<?php echo form_input('order['.$val['id'].']', $val['order'], 'data-module="page" data-id="'.$val['id'].'"  class="form-control sort-order" placeholder="Vị trí" style="width:50px;text-align:right;"');?>
												</td>
												...
												<td class="text-center">
													<a type="button" href="<?php echo site_url('page/backend/page/update/'.$val['id'].'') ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
													<a type="button" class="btn btn-danger ajax-delete" data-title="Lưu ý: Dữ liệu sẽ không thể khôi phục. Hãy chắc chắn rằng bạn muốn thực hiện hành động này!" data-router="" data-module="page" data-id="<?php echo $val['id'] ?>"><i class="fa fa-trash"></i></a>
												</td>
											</tr>
										<?php }}else{ ?>
											<tr>
